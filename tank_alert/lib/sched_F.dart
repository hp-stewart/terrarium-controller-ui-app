...
class Sched_F extends StatefulWidget {
  @override
  _Sched_FState createState() => _Sched_FState();
}

class _Sched_FState extends State<Sched_F> {
  // database variables
  ...
  getBLSchedule() async {
    ...
    List<dynamic> sched = [];
    print("fetching fan schedule from firebase");
    for (var i = 00; i<24; i++) {
      final snapshot = await databaseRef.child("schedule").child("fan").child("${i}").get();
      if (snapshot.value != null) {
                                ...
                                children:[
                                  ...
                                  TextButton(
                                    ...
                                    onPressed: () async {
                                      ...
                                      await databaseRef.child("schedule").child("fan").child("${index}").set(newState);
                                      schedule = await getBLSchedule();
                                      numberOfItems = schedule.length;
                                      setState(() {
