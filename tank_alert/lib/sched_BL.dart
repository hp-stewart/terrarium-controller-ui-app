...
class Sched_BL extends StatefulWidget {
  @override
  _Sched_BLState createState() => _Sched_BLState();
}

class _Sched_BLState extends State<Sched_BL> {
  // database variables
  ...
  getBLSchedule() async {
    ...
    List<dynamic> sched = [];
    print("fetching blue light schedule from firebase");
    for (var i = 00; i<24; i++) {
      final snapshot = await databaseRef.child("schedule").child("blue led").child("${i}").get();
      if (snapshot.value != null) {
                                ...
                                children:[
                                  ...
                                  TextButton(
                                    ...
                                    onPressed: () async {
                                      ...
                                      await databaseRef.child("schedule").child("blue led").child("${index}").set(newState);
                                      schedule = await getBLSchedule();
                                      numberOfItems = schedule.length;
                                      setState(() {
